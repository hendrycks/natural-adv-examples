...
if __name__ == '__main__':  
    ...
    naes = dset.ImageFolder(root="C:\\Users\\Liang\\dataset\\ImageNet-A", transform=test_transform)
    ...
    def get_net_results():
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(nae_loader):
                ...
                # ######### OLD VERSION (Which is NOT CORRECT in terms of robustness evaluation) ############################
                ...
                # ###### This Version gives acc. (using densenet121 pretrained) : 2.1467 %  ############################


                ###### Instead, the robustness evaluation should use the following protocal:  ############################
                logits_full = net(data)
                pred_full = torch.argmax(logits_full, dim=1)  # This predicts 1 out of the original 1000 classes
                
                batch_correct = []
                for i in range(logits_full.shape[0]):   # For every sample i in the batch
                    prediction_200 = thousand_k_to_200[pred_full[i].item()]   # map the 1/1000 prediction into the 200 sub classes, if this is -1, means the model predicts something outside the 200 classes and should be considered a failure
                    label_200 = target[i].item()
                    print("Pred [%d] - Label [%d] " % (prediction_200, label_200))
                    
                    if prediction_200 == label_200:
                        num_correct += 1
                        batch_correct.append(True)
                    else:
                        batch_correct.append(False)
                # correct.extend(batch_correct)

                # == Calculate Softmax Scores ==
                softmax_scores = torch.softmax(logits_full, dim=1)
                softmax_200 = softmax_scores[:, indices_in_1k]
                confidence.extend(to_np(torch.argmax(softmax_200, dim=1)).squeeze().tolist())
                ##### This Version gives acc. (using densenet121 pretrained) : 0.48 %     ###############################
